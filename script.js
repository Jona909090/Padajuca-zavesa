...
function computeLayout() {
  ...
  const cols = vw < 600 ? 31 : vw < 900 ? 42 : 54;
  const lines = wrapAndJustify(MESSAGE, cols);
  const width = Math.min(vw * 0.78, 1000);
  const height = Math.min(vh * 0.97, Math.max(650, lines.length * 32));
  return { cols, rows: lines.length, lines, width, height,
...
function makeGlyphAtlas(fontSize) {
  ...
  for (const ch of chars) {
    ...
    ctx.shadowColor = "rgba(255, 198, 70, 0.72)"; ctx.shadowBlur = 5; ctx.fillText(ch, box / 2, box / 2);
    off.logicalSize = box; atlas[ch] = off;
  }
  return atlas;
}
...
function main() {
  ...
  function draw() {
    const offsetX = (c.width / dpr - layout.width) / 2, offsetY = (c.height / dpr - layout.height) / 2;
    for (const p of particles) {
      if (!p.char || p.char === " ") continue; const img = glyphs[p.char]; if (!img) continue;
      let cos = 1, sin = 0;
      if (p.downConstraint) { const dx = p.downConstraint.p2.pos.x - p.downConstraint.p1.pos.x, dy = p.downConstraint.p2.pos.y - p.downConstraint.p1.pos.y; const a = Math.atan2(dy, dx) - Math.PI / 2; cos = Math.cos(a); sin = Math.sin(a); }
  ...
  let last=0;
  function loop(now) { rafID=requestAnimationFrame(loop); ctx.clearRect(0,0,c.width,c.height); const dt=last?Math.min(now-last,32):16; last=now; particles.forEach(p=>p.update(dt)); for(let k=0;k<CONFIG.iterationsPerFrame;k++) constraints.forEach(con=>con.solve()); draw(); }
  rafID=requestAnimationFrame(loop);
}

class Input {
  constructor(canvas,particles){this.c=canvas;this.particles=particles;this.mouse=new Vec2();this.radius=Math.max(18,layout.cellWidth*1.25);this.bind();}
  setMouse(e){const rect=this.c.getBoundingClientRect(),ox=(this.c.width/dpr-layout.width)/2,oy=(this.c.height/dpr-layout.height)/2;this.mouse.x=e.clientX-rect.left-ox;this.mouse.y=e.clientY-rect.top-oy;}
