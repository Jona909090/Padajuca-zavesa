...
function computeLayout() {
  ...
  return { cols, rows: lines.length, lines, width, height, top,
...
function makeGlyphAtlas(fontSize) {
  ...
  for (const ch of chars) {
    if (ch === " ") continue;
    const off = document.createElement("canvas"); off.width = off.height = Math.ceil(box * dpr);
    const ctx = off.getContext("2d"); ctx.scale(dpr, dpr);
    ctx.font = `italic 600 ${fontSize}px "Cormorant Garamond", Georgia, serif`;
    ctx.textAlign = "center"; ctx.textBaseline = "middle"; ctx.fillStyle = "#f0c85b";
    ctx.shadowColor = "rgba(255, 198, 70, 0.72)"; ctx.shadowBlur = 5;
    ctx.fillText(ch, box / 2, box / 2); off.logicalSize = box; atlas[ch] = off;
  }
  return atlas;
}

function main() {
  if (rafID) cancelAnimationFrame(rafID); if (input) input.unbind(); layout = computeLayout();
  c = document.createElement("canvas"); container.innerHTML = ""; container.appendChild(c); sizeCanvas();
  ...
  for (let i = 0; i < layout.cols; i++) for (let j = 0; j < layout.rows; j++) {
    particles.push(new Particle({ x:i*layout.cellWidth, y:j*layout.cellHeight, pinned:j===0, char:layout.lines[j]?.[i] || " " }));
  }
  for (let i = 0; i < layout.cols; i++) for (let j = 0; j < layout.rows; j++) {
    const p=particles[getPointID(j,i,layout.rows)];
    if(j<layout.rows-1){const down=particles[getPointID(j+1,i,layout.rows)];const vc=new Constraint(p,down,layout.cellHeight,CONFIG.compressFactor,CONFIG.stretchFactor);constraints.push(vc);p.downConstraint=vc;}
    if(i<layout.cols-1) constraints.push(new Constraint(p,particles[getPointID(j,i+1,layout.rows)],layout.cellWidth,.72,3.2));
  }
  input=new Input(c,particles);

  function draw(){
    const offsetX=(c.width/dpr-layout.width)/2, offsetY=layout.top;
    for(const p of particles){
      if(!p.char||p.char===" ")continue;const img=glyphs[p.char];if(!img)continue;
      let cos=1,sin=0;if(p.downConstraint){const dx=p.downConstraint.p2.pos.x-p.downConstraint.p1.pos.x,dy=p.downConstraint.p2.pos.y-p.downConstraint.p1.pos.y,a=Math.atan2(dy,dx)-Math.PI/2;cos=Math.cos(a);sin=Math.sin(a);}
      ctx.setTransform(dpr*cos,dpr*sin,-dpr*sin,dpr*cos,dpr*(p.pos.x+offsetX),dpr*(p.pos.y+offsetY));
      const half=img.logicalSize/2;ctx.globalAlpha=.98;ctx.drawImage(img,-half,-half,img.logicalSize,img.logicalSize);
    }
    ctx.globalAlpha=1;ctx.setTransform(1,0,0,1,0,0);
  }
  let last=0;function loop(now){rafID=requestAnimationFrame(loop);ctx.clearRect(0,0,c.width,c.height);const dt=last?Math.min(now-last,32):16;last=now;particles.forEach(p=>p.update(dt));for(let k=0;k<CONFIG.iterationsPerFrame;k++)constraints.forEach(con=>con.solve());draw();}rafID=requestAnimationFrame(loop);
}

class Input{constructor(canvas,particles){this.c=canvas;this.particles=particles;this.mouse=new Vec2();this.radius=Math.max(18,layout.cellWidth*1.25);this.bind();}setMouse(e){const rect=this.c.getBoundingClientRect(),ox=(this.c.width/dpr-layout.width)/2,oy=layout.top;this.mouse.x=e.clientX-rect.left-ox;this.mouse.y=e.clientY-rect.top-oy;}down(e){this.setMouse(e);for(const p of this.particles)if(this.mouse.subtractNew(p.pos).length<this.radius){this.grabbed=p;p.wasPinned=p.pinned;p.pinned=true;break;}}move(e){this.setMouse(e);if(this.grabbed){this.grabbed.pos.reset(this.mouse.x,this.mouse.y);this.grabbed.oldPos.reset(this.mouse.x,this.mouse.y);}for(const p of this.particles){const diff=this.mouse.subtractNew(p.pos),ls=diff.lengthSquared;if(ls<CONFIG.mouseSize){const a=diff.angle-Math.PI,strength=smoothstep(CONFIG.mouseSize,-2000,ls)*CONFIG.mouseStrength/300;p.applyForce(new Vec2(Math.cos(a)*strength,Math.sin(a)*strength));}}}up(){if(this.grabbed){this.grabbed.pinned=this.grabbed.wasPinned;this.grabbed=null;}}context(e){e.preventDefault();}bind(){this.down=this.down.bind(this);this.move=this.move.bind(this);this.up=this.up.bind(this);this.context=this.context.bind(this);document.addEventListener("pointerdown",this.down);document.addEventListener("pointermove",this.move);document.addEventListener("pointerup",this.up);document.addEventListener("contextmenu",this.context);}unbind(){document.removeEventListener("pointerdown",this.down);document.removeEventListener("pointermove",this.move);document.removeEventListener("pointerup",this.up);document.removeEventListener("contextmenu",this.context);}}
